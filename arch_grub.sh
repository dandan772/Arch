...
if [ "$USER_PASSWORD" != "$USER_PASSWORD_CONFIRM" ]; then
    echo -e "${RED}User passwords do not match!${NC}"
    exit 1
fi
...
pacstrap -K /mnt base base-devel linux linux-lts linux-firmware amd-ucode sudo efibootmgr fish btrfs-progs grub grub-btrfs inotify-tools reflector
...
# Configure mirrors with reflector before installing packages
...
sed -i "s|^GRUB_CMDLINE_LINUX=.*|GRUB_CMDLINE_LINUX=\"cryptdevice=UUID=\$CRYPT_UUID_GRUB:cryptroot root=/dev/mapper/cryptroot rootflags=subvol=@ cryptkey=rootfs:/crypto_keyfile.bin\"|" /etc/default/grub
