...
echo -e "${GREEN}Creating LUKS keyfile...${NC}"
dd bs=512 count=4 if=/dev/random of=/tmp/crypto_keyfile.bin iflag=fullblock
chmod 600 /tmp/crypto_keyfile.bin

echo "Setting up LUKS with keyfile (you'll enter password once to encrypt):"
cryptsetup luksFormat --type luks2 $ROOT_PART /tmp/crypto_keyfile.bin
